...
async function wait(delay) {
    _console$E.log(`waiting for ${delay}ms`);
    return new Promise((resolve) => {
...
const _console$n = createConsole("ColorUtils", { log: false });
function hexToRGB(hex) {
    ...
    _console$n.assertWithError(hex.length == 6, `hex length must be 6 (got ${hex.length})`);
...
function rgbToHex({ r, g, b }) {
    ...
    _console$n.assertWithError([r, g, b].every((v) => v >= 0 && v <= 255), `RGB values must be between 0 and 255 (got r=${r}, g=${g}, b=${b})`);
...
const _console$m = createConsole("DisplayContextStateHelper", { log: false });
class DisplayContextStateHelper {
    ...
    diff(other) {
        ...
        _console$m.log("diff", other, differences);
        return differences;
    }
    update(newState) {
        let differences = this.diff(newState);
        if (differences.length == 0) {
            _console$m.log("redundant contextState", newState);
        }
...
const _console$l = createConsole("DisplayUtils", { log: false });
function formatRotation(rotation, isRadians, isSigned) {
    {
        const rotationRad = rotation;
        _console$l.log({ rotationRad });
        rotation %= 2 * Math.PI;
        rotation /= 2 * Math.PI;
    }
    {
    ...
    _console$l.log({ formattedRotation: rotation });
    return rotation;
}
function roundToStep(value, step) {
    const roundedValue = Math.floor(value / step) * step;
    _console$l.log(value, step, roundedValue);
    return roundedValue;
}
...
function formatBitmapScale(bitmapScale) {
    ...
    _console$l.log({ formattedBitmapScale: bitmapScale });
    return bitmapScale;
}
...
function assertValidSegmentCap(segmentCap) {
    _console$l.assertEnumWithError(segmentCap, DisplaySegmentCaps);
}
function assertValidDisplayBrightness(displayBrightness) {
    _console$l.assertEnumWithError(displayBrightness, DisplayBrightnesses);
}
function assertValidColorValue(name, value) {
    _console$l.assertRangeWithError(name, value, 0, 255);
}
...
function assertValidOpacity(value) {
    _console$l.assertRangeWithError("opacity", value, 0, 1);
}
...
async function quantizeImage(image, width, height, numberOfColors) {
    ...
    _console$k.log({ imageWidth, imageHeight });
    canvas.width = width;
    ...
    _console$k.log("quantOptions", quantOptions);
    _console$k.log("quantizeImage options", quantOptions);
    ...
    _console$k.log("quantized palette data", quantizedPaletteData);
    ...
    _console$k.log({ closestColorIndexToBlack, closestColorDistanceToBlack });
    ...
    _console$k.log("quantizedPaletteColors", quantizedPaletteColors);
    const quantizedColors = quantizedPaletteColors.map((rgb, index) => {
        const hex = rgbToHex(rgb);
        return hex;
    });
    _console$k.log("quantizedColors", quantizedColors);
    ...
    _console$k.log("quantizedColorIndices", quantizedColorIndices);
...
async function resizeAndQuantizeImage(image, width, height, colors) {
    _console$k.assertWithError(colors.length > 1, "colors.length must be greater than 1");
    const canvas = document.createElement("canvas");
    const ctx = canvas.getContext("2d");
    let { naturalWidth: imageWidth, naturalHeight: imageHeight } = image;
    _console$k.log({ imageWidth, imageHeight });
    ...
    quantOptions.palette = colors.map((color) => {
        ...
        if (rgb) {
            ...
            _console$k.error(`invalid rgb hex "${color}"`);
        }
    });
    _console$k.log("quantizeImage options", quantOptions);
    const quantizer = new RGBQuant(quantOptions);
    ...
    for (let i = 0; i < pixels.length; i += 4) {
        ...
        if (colorIndex == -1) {
            _console$k.error(`no color found for ${hex}`);
...
_DisplayCanvasHelper_eventDispatcher = new WeakMap(), _DisplayCanvasHelper_canvas = new WeakMap(), _DisplayCanvasHelper_context = new WeakMap(), _DisplayCanvasHelper_frontDrawStack = new WeakMap(), _DisplayCanvasHelper_rearDrawStack = new WeakMap(), _DisplayCanvasHelper_applyTransparency = new WeakMap(), _DisplayCanvasHelper_device = new WeakMap(), _DisplayCanvasHelper_boundDeviceEventListeners = new WeakMap(), _DisplayCanvasHelper_numberOfColors = new WeakMap(), _DisplayCanvasHelper_colors = new WeakMap(), _DisplayCanvasHelper_opacities = new WeakMap(), _DisplayCanvasHelper_displayContextStateHelper = new WeakMap(), _DisplayCanvasHelper_interval = new WeakMap(), _DisplayCanvasHelper_isReady = new WeakMap(), _DisplayCanvasHelper_clearBoundingBoxOnDraw = new WeakMap(), _DisplayCanvasHelper_brightness = new WeakMap(), _DisplayCanvasHelper_brightnessOpacities = new WeakMap(), _DisplayCanvasHelper_instances = new WeakSet(), _DisplayCanvasHelper_dispatchEvent_get = function _DisplayCanvasHelper_dispatchEvent_get() {
    ...
    const db = darken(b);
    return `rgb(${dr}, ${dg}, ${db})`;
}, _DisplayCanvasHelper_colorIndexToRgba = function _DisplayCanvasHelper_colorIndexToRgba(colorIndex) {
    return __classPrivateFieldGet(this, _DisplayCanvasHelper_instances, "m", _DisplayCanvasHelper_hexToRgba).call(this, this.colors[colorIndex], 1);
