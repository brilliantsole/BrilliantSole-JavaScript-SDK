...
AFRAME.registerComponent("fingertip-colliders", {
  ...
  init: function () {
    ...
    const onHandTrackingExtrasReady = () => {
      ...
      const clearHand = () => {
        const material =
          this.el.components["hand-tracking-controls"]?.skinnedMesh?.material;
        if (material) {
      ...
      this.data.fingers.forEach((finger) => {
        ...
        fingertipEntity.getJointAPI = () =>
          this.jointsAPI[`get${capitalizedfinger}Tip`]();
        //fingertipEntity.setAttribute("obb-collider", this.obbColliderAttributeValue);

        fingertipEntity.addEventListener(
          "obbcollisionstarted",
          this.boundOnOBBCollision
        );
        fingertipEntity.addEventListener(
          "obbcollisionended",
          this.boundOnOBBCollision
    ...
    this.jointsAPI =
      this.el?.components?.["hand-tracking-extras"]?.HandData?.jointAPI;
    if (this.jointsAPI) {
    ...
    this.updateFingertipEntities = AFRAME.utils.throttle(
      this.updateFingertipEntities,
      20,
      this
    );
  },
  ...
  checkIfHandIsVisible: function () {
    ...
    if (this.isHandVisible != isHandVisible) {
      ...
      Object.entries(this.fingertipEntities).forEach(
        ([finger, fingertipEntity]) => {
          if (this.isHandVisible) {
            fingertipEntity.setAttribute(
              "obb-collider",
              this.obbColliderAttributeValue
            );
          } else {
